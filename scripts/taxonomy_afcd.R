...
afcd_taxa <- afcd_taxa %>% 
        ...
        mutate(
          # first make all names lowercase (less character matching required) and remove all the weird spaces and classifications (e.g., "families:__") used in a few of these
          taxa_name = tolower(species), #some of these are genus, family, or higher taxononomic class... so keeping this more general!
          genus = tolower(genus),
...
write.csv(afcd_taxa_names,
    file.path(wk_dir,"data","OutputsFromR","aquatic_food_composition_database","afcd_taxonomic_names_only.csv"),
    row.names=FALSE
  )
